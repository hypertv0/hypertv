...
# --- YENİ ENDPOINT: TEMİZ VAVOO URL'LERİ İÇİN ---
@app.route('/play/<vavoo_id>.m3u8')
def play_vavoo_clean_url(vavoo_id):
    """
    /play/ID.m3u8 formatındaki temiz linkleri kabul edip oynatır.
    Mevcut VavooResolver ve /proxy/ts altyapısını kullanır.
    """
    # Gelen ID'den tam Vavoo URL'sini oluştur
    original_vavoo_url = f"https://vavoo.to/vavoo-iptv/play/{vavoo_id}"
    app.logger.info(f"Yeni endpoint /play: Gelen ID: {vavoo_id}, Oluşturulan URL: {original_vavoo_url}")

    try:
        # Mevcut resolver'ı kullanarak linki çöz
        final_m3u8_url = vavoo_resolver.resolve_vavoo_link(original_vavoo_url)
        if not final_m3u8_url:
            app.logger.error(f"/play - Vavoo linki çözülemedi: {original_vavoo_url}")
            return "Vavoo linki çözülemedi.", 404
        
        app.logger.info(f"Vavoo linki başarıyla çözüldü: {final_m3u8_url}")

        # Çözülen .m3u8 dosyasının içeriğini çek
        headers_for_fetch = {'User-Agent': 'Mozilla/5.0'}
        m3u_response = requests.get(final_m3u8_url, headers=headers_for_fetch, timeout=REQUEST_TIMEOUT)
        m3u_response.raise_for_status()
        m3u_content = m3u_response.text
        
        # .ts linklerini yeniden yazmak için referans alınacak ana URL
        base_url_for_rewrite = m3u_response.url

        # .ts segmentlerini /proxy/ts endpoint'ine yönlendirerek yeniden yaz
        modified_lines = []
        for line in m3u_content.splitlines():
            line = line.strip()
            if line and not line.startswith('#'):
                # Segmentin tam URL'sini oluştur
                absolute_ts_url = urljoin(base_url_for_rewrite, line)
                # Proxy endpoint'i için URL'yi kodla
                encoded_ts_url = quote(absolute_ts_url)
                # Mevcut /proxy/ts endpoint'ini kullanarak yeni linki oluştur
                rewritten_line = f"/proxy/ts?url={encoded_ts_url}"
                modified_lines.append(rewritten_line)
            else:
                # Yorum ve diğer direktifleri olduğu gibi koru
                modified_lines.append(line)

        modified_content = "\n".join(modified_lines)

        # Değiştirilmiş .m3u8 dosyasını kullanıcıya gönder
        return Response(modified_content, mimetype="application/vnd.apple.mpegurl")

    except Exception as e:
        app.logger.error(f"/play endpoint hatası: {e}")
        traceback.print_exc()
        return "Proxy hatası: Kaynak M3U8 alınırken veya işlenirken bir sorun oluştu.", 502
